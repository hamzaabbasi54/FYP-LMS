...
const AssessmentDetails = () => {
    ...
    const getTypeColor = (type) => {
        switch (type) {
            ...
            case 'midterm': return 'bg-amber-50 text-amber-700 border-amber-200';
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-3 mt-1">
                            <span className={`inline-flex px-2 py-0.5 rounded text-[10px] tracking-wider uppercase font-bold border ${getTypeColor(assessment.type)}`}>
                ...
                <div className="flex items-center gap-3">
                    {assessment.status === 'scheduled' ? (
                        <span
                            className="flex items-center px-4 py-2.5 bg-slate-100 text-slate-500 border border-slate-200 rounded-lg shadow-sm font-medium text-sm cursor-not-allowed"
                    ...
                    <Link
                        ...
                        className="flex items-center px-4 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-lg hover:bg-slate-50 shadow-sm transition-colors font-medium text-sm"
                    >
                    ...
                    <button
                        ...
                        className={`flex items-center px-4 py-2.5 rounded-lg shadow-sm transition-colors font-medium text-sm border ${
                            isGraded
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 sm:p-8">
                <h2 className="text-lg font-bold text-slate-800 mb-4 pb-3 border-b border-slate-100">Assessment Information</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    {/* Grading Status */}
                    <div className="flex items-start gap-3">
                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${isGraded ? 'bg-emerald-50' : 'bg-slate-50'}`}>
                    ...
                    <div className="mt-6 pt-4 border-t border-slate-100">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {assessment.mapped_clos.map(clo => (
                                <span key={clo.id} className="inline-flex px-2 py-0.5 rounded text-[10px] uppercase tracking-wider font-bold bg-indigo-50 text-indigo-700 border border-indigo-200">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 sm:p-8">
                ...
                {questions.length === 0 ? (
                    ...
                    <div className="space-y-3">
                        {/* Table Header */}
                        <div className="hidden md:grid grid-cols-12 gap-4 px-4 py-2 bg-slate-50 rounded-lg text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                        ...
                        {questions.map((q, index) => (
                            <div key={q.id || index} className="grid grid-cols-1 md:grid-cols-12 gap-4 px-4 py-3 border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors items-center">
                                <div className="col-span-1">
                                    <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-blue-50 border border-blue-200 text-blue-700 text-sm font-bold">
                                ...
                                <div className="col-span-2">
                                    {q.clo_title ? (
                                        <span className="inline-flex px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-indigo-50 text-indigo-700 border border-indigo-200">
                                            {q.clo_title}
                                        </span>
                                    ) : (
